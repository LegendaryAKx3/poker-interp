...
def run_probing(ckpt_dir: str, tokenizer_dir: str, data_dir: str, max_samples: int = 10000, hidden_dim: int = 256, epochs: int = 10, batch_size: int = 64):
    ...
    # ---------------------------
    os.makedirs("confusion_matrices/HandRankSelectEquated3MLP200kTest", exist_ok=True)
    ...
    for layer_idx, (h_train, h_test) in enumerate(zip(hidden_train, hidden_test)):
        ...
        with torch.no_grad():
            ...
            plt.tight_layout()
            plt.savefig(f"confusion_matrices/HandRankSelectEquated3MLP200kTest/confusion_matrix_layer_{layer_idx:02d}.png", dpi=300)
            plt.close()

    print("MLP probing complete. Confusion matrices saved in 'confusion_matrices/HandRankSelectEquated3MLP200kTest/'.")
