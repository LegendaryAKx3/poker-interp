...
# Modal App Setup
# ---------------------------
app = modal.App("pokerGPT-activation-visualization")
...
def run_activation_visualization(ckpt_dir: str, tokenizer_dir: str, data_dir: str, max_samples: int = 200000):
    os.chdir("/data")
    ...
    for layer_idx, hidden_layer in enumerate(hidden_states):
        ...
        hidden_pca = pca.fit_transform(hidden_avg)

        # t-SNE
        tsne = TSNE(n_components=2, random_state=42, init="pca", learning_rate="auto")
        hidden_tsne = tsne.fit_transform(hidden_avg)

        # UMAP
        reducer = umap.UMAP(n_components=2, random_state=42)
        hidden_umap = reducer.fit_transform(hidden_avg)

        # Plot helper
        def plot_scatter(data, method_name):
            plt.figure(figsize=(8,6))
            cmap = plt.cm.get_cmap("tab10", len(unique_labels))
            for i, rank in enumerate(unique_labels):
                idx = np.array(y) == rank
                plt.scatter(data[idx,0], data[idx,1], label=rank, alpha=0.7, color=cmap(i))
            plt.title(f"{method_name} of Layer {layer_idx} Activations")
